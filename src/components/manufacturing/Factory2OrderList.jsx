...
const Factory2OrderList = () => {
    ...
    // 2공장 데이터만 필터링
    let factory2Orders = (factory2OrdersFromRedux || []).filter(order => order.factoryId === 2);
    ...
    if (statusFilter !== '전체') {
        ...
        factory2Orders = factory2Orders.filter(order => order.status === statusMap[statusFilter]);
    }
    ...
    const handleSubmitCompletion = () => {
        ...
        // quantity에서 숫자 부분만 추출 (예: "300 box" -> 300)
        const quantityValue = parseInt(selectedOrder.quantity);
        const completedQuantity = `${quantityValue - parseInt(completionData.damage)} ${selectedOrder.quantity.split(' ')[1] || 'box'}`;
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-white rounded-lg p-3 border border-[#674529]/20">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="text-base font-bold text-gray-700">
                                                {order.completionInfo.completedQuantity}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white rounded-lg p-3 border border-[#674529]/20">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="text-base font-bold text-gray-700">
                                                {order.completionInfo.damage} box
                                            </span>
                                        </div>
                                    </div>
